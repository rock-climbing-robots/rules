...
The event combines mechanical design (limbs, grippers, harness), control
...
The robot must be \emph{legged}: it climbs using its articulated limbs. Other
parts of the robot, such as its trunk, may also touch the wall and the holds.

The robot must have a \emph{humanoid-type body}: a central trunk carrying
two upper limbs and two lower limbs.

Whether a robot is indeed humanoid is left to the discretion of the
organisers.

A robot must have \textbf{no more than four limbs}.
...
load-bearing: it shall not be able to hold the robot if the harness fails, and
...
\textbf{Four walls} are provided by the organisers, one for each
\textbf{difficulty level, from 1 to 4}. The difficulty comes from the holds set
on the wall: sparsity of the holds, paths that are not climbable and must be
discarded by the planner, etc.
...
The board is drilled with a regular square grid of \textbf{M4} holes with
a pitch of \textbf{25\,mm}. The border margin is 25\,mm on all four sides.
...
\begin{itemize}
  \item Board inclination: strictly vertical, or slightly overhanging?
...
\begin{itemize}
  \item a hold is fixed with \textbf{two M4 bolts}, on \textbf{two holes side
        by side horizontally} on the grid, i.e.\ \textbf{25\,mm apart}; the bolts go through the hold
        and the board, and are tightened by \textbf{nuts on the opposite side
        of the wall};
  \item a hold must fit inside a \textbf{cylinder of radius 60\,mm and length
...
\begin{figure}[h!]
  ...
  \caption{Hold specification: two M4 bolts on horizontally adjacent grid holes, passing
           through the hold and the board and tightened by nuts on the rear of
           the wall, and the cylindrical envelope ($R=60$\,mm, $L=100$\,mm) the
...
\begin{figure}[h!]
  ...
  \caption{Isometric view of the setup: instrumented wall, extension and
...
\subsection{Safety rope}

A safety rope is provided for every robot.
It runs over a pulley carried by an \textbf{extension fixed at the top of the
wall}.
Its use is \textbf{mandatory} for any run on the wall.

\textbf{The rope is handled by the team itself.} No belay or tether
management device is provided: a team member is responsible for handling the
rope during the whole run, so that the robot is caught before it reaches the
ground.

The rope must be kept slack enough not to assist the robot, and tight
enough to limit the fall. 
...
loads the harness, never the connectors.
...
\begin{itemize}
  \item The simulated competition uses walls of the \textbf{same four
        difficulty levels} as the real one.
...
of four difficulty levels. The goal is to \textbf{climb the wall in the
shortest time}.
...
\midrule
Wall difficulty level 4 & \tf{tfA}{1}\\
Wall difficulty level 3 & \tf{tfB}{1.1}\\
...
Factors multiply: for instance, a run on a level-3 wall, starting at the bottom
of the wall and using markers, has its time multiplied by
$1.1\times1.1\times1.25=1.5125$.
